...
public struct Details
{
	...
	private Dictionary<EventManagerOriginal.DialogueAction, DialogueOptions> allOptions;
	...
	public void SetDialogueAction(EventManagerOriginal.DialogueAction action)
	{
        DialogueOptions newDialogue = new DialogueOptions(action.ToString());
        if(allOptions == null)
        {
			allOptions = new Dictionary<EventManagerOriginal.DialogueAction, DialogueOptions>();
        }
	...
	public EventManagerOriginal.DialogueAction GetNextDialogueAction()
    {
		foreach (KeyValuePair<EventManagerOriginal.DialogueAction, DialogueOptions> entry in allOptions)
        {
          
            return entry.Key;
            
        }
		return EventManagerOriginal.DialogueAction.None;

    }

	public void SetDialogueAsComplete(EventManagerOriginal.DialogueAction finished)
    {
        bool remove = false;
		foreach (KeyValuePair<EventManagerOriginal.DialogueAction, DialogueOptions> entry in allOptions)
        {
...
public class EventManagerOriginal : MonoBehaviour
{
	...
	private static EventManagerOriginal eventManager;


	[SerializeField]
	public SenderEvent DoneEvent;

	public static EventManagerOriginal instance
    {
        get
        {
            if(!eventManager)
            {
				eventManager = FindObjectOfType(typeof(EventManagerOriginal)) as EventManagerOriginal;
